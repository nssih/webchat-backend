...
public class GroupController {
    ...
    public GroupController(GroupService groupService, ChatWebSocketHandler wsHandler, ObjectMapper objectMapper) {
    ...
    public ApiResponse<GroupResponse> invite(
        ...
        GroupService.InviteResult result = groupService.inviteMember(groupId, principal.getId(), userId);
        // 事务已提交，安全发送 WS 通知给被邀请人
        try {
            String groupJson = objectMapper.writeValueAsString(result.group());
            wsHandler.sendToUsername(result.invitedUsername(), WsMessage.builder()
                    .type(WsMessage.Type.GROUP_MEMBER_ADDED)
                    .groupId(groupId)
                    .groupData(groupJson)
                    .timestamp(System.currentTimeMillis())
                    .build());
        } catch (Exception e) {
            log.warn("Failed to send GROUP_MEMBER_ADDED to {}: {}", result.invitedUsername(), e.getMessage());
        }
        return ApiResponse.ok(result.group());
    }
    ...
    public ApiResponse<String> getMyGroupKey(
        ...
        if (version != null) {
            return ApiResponse.ok(groupService.getGroupKey(groupId, username, version));
        }
